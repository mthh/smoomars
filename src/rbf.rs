...
impl<'a, T> Rbf<'a, T> where T: PtValue {
    pub fn new(obs_points: &'a [T], distance_function: &str, epsilon: Option<f64>) -> Self {
        ...
        for j in 0..nb_pts {
            for i in 0..nb_pts {
                mat[j * nb_pts + i] = __norm::<T>(&obs_points[i], &obs_points[j]);
            }
        }
        ...
        Rbf {obs_points: obs_points, distance_function: distance_func, epsilon: eps, weights: weights}
    }
...
pub fn rbf_interpolation<T>(reso_x: u32,
                            ...
                            obs_points: &[T],
                            func_name: &str,
                            epsilon: Option<f64>) -> Result<Vec<T>> where T: PtValue
{
...
// fn norm(pa: f64, pb: f64) -> f64 {
//     (pa - pb).abs()
// }

fn __norm<T>(pa: &T, pb: &T) -> f64 where T: PtValue {
    let ca = pa.get_coordinates();
    let cb = pb.get_coordinates();
    ((ca.0 - cb.0).powi(2) + (ca.1 - cb.1).powi(2)).sqrt()
}

fn _norm(pa: &[f64], pb: &[f64]) -> f64 {
    let mut r: f64 = 0.0;
    for i in 0..pa.len() {
        r += (pa[i] - pb[i]).powi(2);
    }
